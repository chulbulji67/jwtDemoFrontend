...
export default function Home() {
  ...
  const [booksPerPage] = useState(9); // Number of books to display per page
  ...
  useEffect(() => {
    ...
    .catch(bookError => {
      setError("Error fetching books " + bookError.message); // Set the error message
    });
  }, []);
      ...
      <div className="flex justify-center mt-4">
        {Array.from({ length: Math.ceil(books.length / booksPerPage) }, (_, i) => i + 1).map(number => (
          <button
            ...
            className={`mx-1 px-3 py-1 rounded-lg  mb-2 ${currentPage === number ? 'bg-blue-500 text-white' : 'bg-gray-300 text-black'}`}
          >
            {number}
          </button>
        ))}
